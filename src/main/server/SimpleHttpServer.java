...
public class SimpleHttpServer {
    ...
    private void continueUpload(SelectionKey key, SocketChannel client,
        ...
        // Write only as many bytes as we still need (don't overflow into next request)
        ...
        if (state.written >= state.totalBodyBytes) {
            // All bytes received — parse and respond
            finishUpload(key, client, state);
        }
        // else: stay in UploadState, wait for more chunks
    }
